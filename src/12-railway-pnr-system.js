...
export function processRailwayPNR(pnrData) {
    ...
    if (typeof pnr !== "string" || pnr.length !== 10 || !/^\d+$/.test(pnr))
        return null;
    const pnrFormatted =
    ...
    const trainInfo = `Train: ${train.number} - ${train.name} | ${train.from} → ${train.to} | Class: ${pnrData.classBooked}`;
    ...
    const processedPassengers = passengers.map((passenger) => {
        ...
        let statusLabel;
        if (currentStatus.startsWith("B")) {
            statusLabel = "CONFIRMED";
        } else if (currentStatus.startsWith("S")) {
            statusLabel = "CONFIRMED";
        } else if (currentStatus.startsWith("WL")) {
            statusLabel = "WAITING";
        } else if (currentStatus.startsWith("CAN")) {
            statusLabel = "CANCELLED";
        } else if (currentStatus.startsWith("RAC")) {
            statusLabel = "RAC";
        }
    ...
    const confirmed = processedPassengers.filter(
        (passe) => passe.statusLabel === "CONFIRMED",
    ).length;
    const waiting = processedPassengers.filter(
        (passe) => passe.statusLabel === "WAITING",
    ).length;
    const cancelled = processedPassengers.filter(
        (passe) => passe.statusLabel === "CANCELLED",
    ).length;
    const rac = processedPassengers.filter(
        (passe) => passe.statusLabel === "RAC",
    ).length;

    const allConfirmed = processedPassengers.every(
        (passe) => passe.statusLabel === "CONFIRMED",
    );
    ...
    const nonCancelled = processedPassengers.filter(
        (passe) => passe.statusLabel !== "CANCELLED",
    );
    const chartPrepared = nonCancelled.every(
        (passe) => passe.statusLabel === "CONFIRMED",
    );
